...
```sh
python3 pipeline/standardise_and_rearrange_data.py --config configs/standardise_and_rearrange_data.yml
```
...
```sh
python3 pipeline/make_input.py --config configs/make_input.yml
```

We now have a nice 3D tensor of shape (sample size, max sequence length, feature size) as input. 

## Generate synthetic data
Due to specific circumstances we needed to generate synthetic data from the original DPV data, and used both datasets in parallel throughout the analysis. Here we implemented the conditional variational autoencoder on top of the long short term memory modeling (LSTM-VAE) to robustly preserving the original data distribution and the autoregressive data structure while introducing enough noise to make new data. 

To generate synthetic data, simply edit the `configs/generate_synthetic_data.yml` file and then run the following script:

```sh
python3 pipeline/run_lstm_vae.py --config configs/generate_synthetic_data.yml
```
...
```sh
python3 pipeline/run_prime_llm.py --config configs/run_prime_llm.yml
```
...
```sh
python3 experiments/cv_prime_llm_trainSize.py --config configs/cv_prime_llm_trainSize.yml
python3 experiments/cv_transformer_trainSize.py --config configs/cv_transformer_trainSize.yml
```
...
```sh
python3 experiments/cv_prime_llm_slidWindow.py --config configs/cv_prime_llm_SlidWindow.yml
python3 experiments/cv_xgb_slidWindow.py --config configs/cv_xgb_slidWindow.yml
```
...
```sh
python pipeline/get_integrated_gradient.py --config configs/get_integrated_gradient.yml
```
